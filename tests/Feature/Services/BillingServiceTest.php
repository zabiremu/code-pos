 ...
 * beyond the one happy-path bill in Feature\Pos\OrderFlowTest: per-item tax
 * overrides, discounts (percent and fixed, capped at the subtotal), service
 * charges, and split billing by a subset of order items.
 */
class BillingServiceTest extends TestCase
{
    use RefreshDatabase;

    private function orderWithItem(float $basePrice, ?float $itemTaxRate = null, float $branchTaxRate = 10, int $quantity = 1): Order
    {
        ...
        $order = Order::create(['type' => 'takeaway', 'guest_count' => 1, 'status' => 'open']);
        $order->items()->create([
    ...
    {
        $order = $this->orderWithItem(basePrice: 100, itemTaxRate: null, branchTaxRate: 15);

        $bill = app(BillingService::class)->createBill($order);
    ...
    public function test_a_menu_items_own_tax_rate_overrides_the_branch_default(): void
    {
        ...
        // whenever a menu item's own tax_rate was set to exactly 0.
        $order = $this->orderWithItem(basePrice: 100, itemTaxRate: 5, branchTaxRate: 15);
    ...
    {
        $order = $this->orderWithItem(basePrice: 100, itemTaxRate: 0, branchTaxRate: 0);
        $discount = Discount::create([
            'name' => '10% off', 'code' => 'TENOFF', 'type' => 'percent', 'value' => 10, 'is_active' => true,
        ]);

        $bill = app(BillingService::class)->createBill($order, discount: $discount);
    ...
    {
        $order = $this->orderWithItem(basePrice: 20, itemTaxRate: 0, branchTaxRate: 0);
        $discount = Discount::create([
    ...
    public function test_service_charge_is_added_on_top_of_the_subtotal(): void
    {
        ...
        $bill = app(BillingService::class)->createBill($order, serviceChargeRate: 5);
    ...
    {
        $order = $this->orderWithItem(basePrice: 10, itemTaxRate: 0, branchTaxRate: 0, quantity: 1);
        // A second, separate line item on the same order that we will leave unbilled.
        $order->items()->create([
        ...
        $firstItemId = $order->items()->first()->id;

        $bill = app(BillingService::class)->createBill($order, itemIds: collect([$firstItemId]));
    ...
    {
        $order = $this->orderWithItem(basePrice: 50, itemTaxRate: 0, branchTaxRate: 0);
        $bill = app(BillingService::class)->createBill($order);
